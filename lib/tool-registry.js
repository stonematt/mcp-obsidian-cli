 ...
 *   - json:        Optional opt-in (see #29). When true, the registration loop
 *                  appends `format=json` to the built args and runTool returns
 *                  the parsed result as MCP `structuredContent` (degrading to
 *                  text on parse failure). Only set on verbs whose live
 *                  manifest advertises a `format=json` flag.
 */
export const TYPED_TOOL_ENTRIES = [
  ...
  // No `content` param here — intentional, not an oversight. The underlying
  // `templater:create-from-template` verb accepts only template/file/open; it
  // has no body passthrough (verified against live CLI help, 2026-05-24).
  // Don't "fix" this by routing to the `create` verb with template=+content=:
  // that path copies raw bytes and never runs Templater expansion (the bug
  // removed in 1.4.0). There is no single verb that expands AND sets body.
  //
  // A one-call "template + body" is doable only as create-then-append
  // orchestration — but with a trap: `file=` is advisory. Templater relocates
  // the note per its new-file-folder / `tp.file.move` settings, so the created
  // path is NOT what you passed. The verb echoes the REAL path on stdout; any
  // append step must parse that, not reuse `file=`. Tested working but judged
  // not worth the fragile stdout-parse + partial-failure handling for one
  // saved call. Keep the two-step (create_from_template, then append) explicit.
  {
  ...
  },
  {
    ...
    schema: {
      ...
      value: z
        .string()
        .describe(
          'Property value. For type="list", pass a JSON-array string, e.g. \'["a", "b"]\'',
        ),
      type: z
        .enum(["text", "list", "number", "checkbox", "date", "datetime"])
        .optional()
        .describe(
          'Property type. Use "list" for multi-valued props (tags/aliases/Links) with a JSON-array value; omitting it writes a scalar string',
        ),
      file: z.string().optional().describe("File name"),
      path: z.string().optional().describe("File path"),
    },
    requires: ["fileOrPath"],
    build: ({ name, value, type, file, path }) => {
  ...
  },
  {
    ...
    schema: {
      id: z.string().describe("Obsidian command ID to execute"),
    },
    build: ({ id }) => ["command", `id=${id}`],
  },
  {
  ...
  },
  {
    ...
    schema: {
      ...
      resolve: z
        .boolean()
        .optional()
        .describe("Resolve/expand template variables"),
      title: z
        .string()
        .optional()
        .describe("Title for variable resolution (used with resolve)"),
    },
    build: ({ name, resolve, title }) => {
...
export function registerTypedTools(server, runTool, entries, ctx = {}) {
  for (const entry of entries) {
    ...
    server.tool(entry.name, entry.description, entry.schema, async (args) => {
      ...
      // JSON opt-in (see #29): append `format=json` to array-shaped args and
      // ask runTool to parse the result into structured content.
      const finalArgs =
        entry.json && Array.isArray(built) ? [...built, "format=json"] : built;
      return runTool(finalArgs, { json: !!entry.json });
    });
  }
}
