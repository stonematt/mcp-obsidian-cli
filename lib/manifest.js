...
// Sections whose body lines are actual verbs. Every other flush-left
// section (`Options:`, `Notes:`, or anything future help text adds) is
// prose and is structurally excluded from verb/flag collection — see the
// whitelist check in parseHelpOutput.
const VERB_SECTIONS = new Set(["Commands", "Developer"]);
 ...
 *
 * Real help output also has flush-left `Options:` and `Notes:` sections —
 * prose, not verb listings — that happen to indent their body text by the
 * same 2 spaces as a verb line. The heading regex alone can't tell those
 * apart from `Commands:`/`Developer:`, so verb (and flag) collection is
 * additionally gated on a section whitelist: only `Commands` and
 * `Developer` are ever scanned for verb/flag lines. Every other section's
 * body is inert, regardless of what it looks like.
 *
 * @param {string} stdout
...
export function parseHelpOutput(stdout) {
  ...
  for (const raw of lines) {
    ...
    // e.g. "Commands:", "Developer:". Also matches prose section headings
    // like "Options:"/"Notes:" — those are deliberately not filtered out
    // here so `section` always reflects the real current heading; the
    // whitelist below is what keeps their bodies from being read as verbs.
    ...
    // Only `Commands:` and `Developer:` list actual verbs. `Options:` and
    // `Notes:` are prose whose indented body lines otherwise match the
    // verb-line shape below (e.g. "vault=<name>  Target a specific vault by
    // name", or a "file resolves by name..." explanation) and would
    // otherwise be mistaken for real verbs.
    if (!VERB_SECTIONS.has(section)) continue;
